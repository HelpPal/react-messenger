 ...
 *
 * @example
 *     var latlon = decode('u120fxw'); // latlon: { lat: 52.205, lon: 0.1188 }
 */
const decode = (geohash) => {
  const bonds = bounds(geohash); // <-- the hard work
  // now just determine the centre of the cell...

  const latMin = bonds.sw.lat;
  const lonMin = bonds.sw.lon;
  const latMax = bonds.ne.lat;
  const lonMax = bonds.ne.lon;
 ...
 *
 * @example
 *     var geohash = encode(52.205, 0.119, 7); // geohash: 'u120fxw'
 */
 ...
 * @throws  Invalid geohash.
 */
const adjacent = (geohash, direction) => {
